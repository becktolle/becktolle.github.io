...
```{r, echo=FALSE}
ggplot(outer_space_objects, aes(x=Year, y=num_objects))+
  geom_line()+
  theme_minimal()+
...
```
World makes up over half of all objects launched into space.
...
```{r, echo=FALSE}
ggplot(outer_space_objects, aes(x=Year, y=num_objects, Color=Organization))+
...
```{r, echo=FALSE}
ggplot(countobj, aes(fill=Continent))+
...
ggplot(countobj, aes(color=Continent))+
